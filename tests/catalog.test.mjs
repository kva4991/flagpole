...
describe('project catalog', () => {
  ...
  it('uses continuous component IDs and one selected climate module', () => {
    ...
    assert.deepEqual(ids, Array.from({ length: 18 }, (_, index) => `cmp-${String(index + 1).padStart(3, '0')}`));
  ...
  it('contains categories, requested columns, drawings and lazy 3D controls', () => {
    ...
    for (const category of ['Управление и датчики', 'Питание и коммутация', 'Подсветка', 'Механика', 'Ткани', 'Материалы для 3D-печати']) {
