...
def render():
    ...
    ax.set_axis_off()
    ax.set_title('Маршрут питания флага: герморазъём → наружная канавка → электроника',fontsize=17,pad=20)
    fig.canvas.draw()

    callouts=[
        ((81,-12.3,18),'1. Герметичный 2-pin разъём\nпод спицей, рядом с флагом',(0.77,0.66)),
        ((45,-12.3,18),'2. Провод лежит в открытой\nнаружной канавке ниже спицы',(0.72,0.45)),
        ((4,-19,18),'3. Канавка огибает корпус\nпо внешней нижней стороне',(0.35,0.26)),
        ((-12,0,18),'4. Разрезная TPU 95A-втулка\nв стенке электронного бокса',(0.22,0.54)),
        ((-28,-5,22),'5. Внутри — короткая сервисная петля\nи разгрузка до клемм электроники',(0.20,0.76)),
    ]
    ...
    fig.text(0.5,0.045,
             'Кабель не проходит через силовую втулку спицы и не пересекает подшипниковые посадки. '
             'В канавке он фиксируется съёмно: редкими точками нейтрального RTV или короткими TPU-ремешками.',
