...
function validate(source, media, identity, registry, currentVersion) {
  ...
  if (media.schemaVersion !== 4) failures.push('schemaVersion drawings.json должен быть равен 4');
...
const drawings = media.drawings.map(item => {
  ...
  <div class="media-actions"><a class="action" href="${escape(repoUrl(fullPreview))}" target="_blank" rel="noopener">${mainAction}</a>${item.calloutMode === 'overlay' ? `<a class="secondary-action" href="${escape(repoUrl(item.file))}" target="_blank" rel="noopener">Исходник без выносок</a>` : ''}</div>${renderCalloutLegend(item)}</div>
</article>`;
}).join('');
...
const overlay=document.getElementById('fullscreenOverlay'),fullscreenViewer=document.getElementById('fullscreenViewer'),fullscreenTitle=document.getElementById('fullscreenTitle'),fullscreenToggleIds=document.getElementById('fullscreenToggleIds'),fullscreenClose=document.getElementById('fullscreenClose');
