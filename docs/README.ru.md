# Карта документации Crucian

Документация разделена по роли, чтобы новая сессия не перечитывала весь репозиторий и не путала планы с реализованным состоянием. §faststart

## Точки входа

- [Быстрый старт агента](agent-fast-start.ru.md) — минимальный маршрут чтения.
- [Текущий статус](current-implementation-status.ru.md) — что реализовано, проверено или ещё требует стенда.
- [Аудит v0.6.1](AUDIT_V061_RU.md) — найденные риски и границы доказательств.
- [Процесс ведения документации](DOCUMENTATION_WORKFLOW_RU.md) — README, focused docs, ADR, §-теги и quality gate.
- [Архитектура](architecture/README.ru.md) — карта подсистем и принятых решений.
- [Карта §-тегов](dev/tag-map.md) — стабильные связи между правилами, кодом и проверками.

## Тематические документы v0.6.1

- [Сводка проектных решений](PROJECT_DECISIONS_RU.md)
- [Обоснование прошивки и Android](CODE_RATIONALE_RU.md)
- [История решений v0.6.1](CHANGELOG_DECISIONS_V061_RU.md)
- [Исходный handoff](NEXT_CHAT_HANDOFF_RU.md)

## Специализированные разделы

- [Электроника и прошивка v0.6](../electronics/README_V06_RU.md)
- [Android-прототип](../android/README_RU.md)
- [Каталог компонентов](../catalog/README_RU.md)
- [Механическая сборка ASA/PETG/TPU](../mechanical/docs/ASSEMBLY_PETG_TPU_RU.md)
- [Печать: ASA как финальный выбор и профиль PETG v0.5](../mechanical/docs/PRINTING_PETG_TPU_RU.md)
- [Обязательные измерения](../MEASUREMENTS_REQUIRED_RU.md)

Исторические документы v0.5 и v0.2 сохраняются как происхождение решений, но не должны автоматически переопределять ADR и текущий статус v0.6.1.
