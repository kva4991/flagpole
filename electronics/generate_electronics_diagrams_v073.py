#!/usr/bin/env python3
"""Generate current v0.7.3 electronics SVG/PNG diagrams.
...
<text x="148.5" y="10" text-anchor="middle" class="title">Crucian v0.7.3 — актуальная схема соединений электроники</text>
...
''']
    ...
    parts.append(box(224,137,60,30,"XUNATA 12 В Ø16",["одноцветный круглый неон","по контуру ихтиса","ток и нагрев измерить"],"load"))
    parts.append(arrow(260,118,254,137,"2 провода","wire12"))

    parts.append('<rect x="8" y="177" width="276" height="22" rx="3" fill="#fff" stroke="#c6d0d4" stroke-width=".5"/>')
    notes=[
    ...
    for i,n in enumerate(notes): parts.append(f'<text x="13" y="{183+i*4.3}" class="note">{esc(n)}</text>')
    parts.append('</svg>')
...
<rect width="{W}" height="{H}" fill="#f5f6f7"/>
<text x="148.5" y="10" text-anchor="middle" class="title">Crucian v0.7.3 — карта клемм, GPIO и разъёмов</text>
...
''']
    ...
    notes=[
      ...
      "Значения 3,3 В, GPIO3/4/5 и адреса датчиков являются текущей схемой прошивки v0.7.3.",
...
if __name__ == '__main__':
    wiring(); terminal_map(); print('Generated v0.7.3 electronics diagrams.')
